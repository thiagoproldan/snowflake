...
let
  ...
  probe =
    config.perSystem
      ...
      (throw ''
        For the purpose of finding out whether an option may be unset for all systems, snowflake probes the perSystem module without a valid `system` argument, and tries to catch this exception if it finds that `system` is required for this determination. If you see this message, it means that for some reason, snowflake was unable to catch this exception.

        This may be a bug in Nix or in snowflake, but ultimately this is due to the quirky requirement of flakes that the "system" attribute does not come first.
        ...
        (mapAttrs
          (k: v:
            if v.formatter != null then v.formatter
              ...
              throw ''
                snowflake could not determine statically that no formatter is defined for *all* systems.

                What happened?

                1. For performance reasons, snowflake must not query `perSystem` for every system, so it uses a heuristic to determine whether a formatter is defined for all systems.
                ...
                As a consequence of (1), snowflake had to provide the output attribute `formatter.${k}`, but as a consequence of (2), you're seeing this error.

                What to do?

                This whole situation should be temporary. `nix flake check`/`show` can be changed to allow `null` values, which gives snowflake and other frameworks a way to avoid this situation.

                To change the `formatter` output attribute, you can control it precisely with the `touchup` module, for example:

                    imports = [ inputs.snowflake.flakeModules.touchup ];
