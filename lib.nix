...
let
  ...
  /**
    ...
    whereas this version returns a list. The snowflake version was not updated to
  ...
  errorExample = ''
    For example:
        ...
        outputs = inputs@{ snowflake, ... }:
          snowflake.lib.mkFlake { inherit inputs; } { /* module */ };
  ...
  snowflake-lib = rec {
    evalFlakeModule =
      ...
      , self ? inputs.self or (throw ''
          When invoking snowflake, you must pass all the flake output arguments,
      ...
      throwIf
        ...
        When invoking snowflake, you must pass in the flake output arguments.

        ${errorExample}
      ''
        warnIf
        (!args?inputs) ''
        When invoking snowflake, it is recommended to pass all the flake output
      ...
      ''
        ...
        lib.evalModules {
          specialArgs = {
            inherit self snowflake-lib moduleLocation;
    ...
    mkFlake = args: module:
      let
        eval = snowflake-lib.evalFlakeModule args module;
      in
    ...
    /**
      ...
      In snowflake, `flake` is declared as a submodule option by the core modules,
    ...
    */
    mkDeferredModuleOption =
      module:
      mkOption {
        type = snowflake-lib.mkPerSystemType module;
      };
    ...
    */
    ...
    mkTransposedPerSystemModule = { name, option, file }: {
      ...
      options = {
        ...
        perSystem = snowflake-lib.mkPerSystemOption {
          _file = file;
...
then
  abort ''
    The nixpkgs-lib dependency of snowflake was overridden but is too old.
